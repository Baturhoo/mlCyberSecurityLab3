...
def repair(model):
    ...
    for output in yhat:
        for node in range(len(yhat[0][0, 0])):
            contribution_dict[node] = 0
            for i in range(len(yhat[0])):
                for j in range(len(yhat[0][0])):
                    contribution_dict[node] += output[i][j][node]
    contribution_sorted = [(val, key) for key, val in contribution_dict.items()]
    ...
    for i in range (1,20):
        weights[:, :, :, contribution_sorted[i][1]] = np.zeros(np.shape(weights[:, :, :, contribution_sorted[i][1]]))
        yrepair = np.argmax(model.predict(x_valid), axis=1)
        acc = np.mean(np.equal(yrepair, y_valid)) * 100
        acc_dict[i] = acc
        bias[contribution_sorted[i][1]] = 0
