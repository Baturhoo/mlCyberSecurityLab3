        ...
        for node in range(len(yhat[0][0, 0])):
            ...
            for i in range(len(yhat[0])):
                for j in range(len(yhat[0][0])):
                    score[node] += output[i][j][node]
    score_sort = [(val, key) for key, val in score.items()]
    score_sort.sort()
